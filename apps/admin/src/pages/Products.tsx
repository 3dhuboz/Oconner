              ...
              <tr key={p.id} className="hover:bg-gray-50">
                <td className="px-4 py-3">
                  {(p as any).imageUrl && !isStaleUrl((p as any).imageUrl) ? (
                    <img src={(p as any).imageUrl} alt={p.name} className="rounded-xl object-cover bg-gray-100" style={{ width: 80, height: 60, minWidth: 80 }} onError={(e) => { (e.currentTarget as HTMLImageElement).style.display = 'none'; }} />
                  ) : (
                    <div className="rounded-xl bg-gray-100 flex items-center justify-center" style={{ width: 80, height: 60, minWidth: 80 }}>
